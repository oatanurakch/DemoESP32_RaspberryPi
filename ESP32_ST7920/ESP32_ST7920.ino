...
void setup(void) {
  ...
  xTaskCreatePinnedToCore(LED, "LED", 1000, NULL, 1, NULL, 1);
  xTaskCreatePinnedToCore(ReadDS, "ReadDS", 1000, NULL, 1, NULL, 0);
//  LEDC setup
  ledcSetup(0, 5000, 8);          
  ledcSetup(1, 5000, 8);
  ledcSetup(2, 5000, 8);
...
void LED(void *pvParameter){
  while(1){
    ...
    int ledst = Serial.readString().toInt();
    if(ledst == 1){
      digitalWrite(LED9, 1);
    }else{
      digitalWrite(LED9, 0);
...
void ReadDS(void *pvParameter){
  while(1){
    sensors.requestTemperatures(); 
    temperatureC = sensors.getTempCByIndex(0);
    humid = random(7000,10000) / 100.0;
